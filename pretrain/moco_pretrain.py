...
parser.add_argument('--data', metavar='DIR', default='./datasets/TRAIN.csv',
                    help='path to dataset')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    model = moco.builder.MoCo(
        base_model,
        args.moco_dim, args.moco_k, args.moco_m, args.moco_t, args.mlp, device=args.gpu)
    print(model)
    ...
    train_dataset = TextDataset(args.data, True, 144)  # 'TRAIN.csv+TEST.csv'
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        losses, top1, top5 = train(train_loader, model, criterion, optimizer, epoch, args, ngpus_per_node)
        ...
        if not args.multiprocessing_distributed or (args.multiprocessing_distributed
            ...
            if (epoch+1) % args.save_freq == 0:
                ...
                torch.save( {'net': model.module.encoder_q.state_dict()}, f'{args.save_folder}/encoder_q.tar')
...
def train(train_loader, model, criterion, optimizer, epoch, args, ngpus_per_node):
    ...
    for i, images in enumerate(train_loader):
        ...
        if args.gpu is not None and False:
            images['x'] = images['x'].cuda(args.gpu, non_blocking=True)
            images['x2'] = images['x2'].cuda(args.gpu, non_blocking=True)

        # compute output
        output, target = model(im_q=images['x'], im_k=images['x2'], epoch=epoch)
        ...
        losses.update(loss.item(), len_x)
        top1.update(acc1[0], len_x)
        top5.update(acc5[0], len_x)
